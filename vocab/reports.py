...
def interferometer_report(path: str = ".", target: str = "",
                            aperture: int = 2, max_arrays: int = 8) -> dict:
    """Interferometric Array — sparse cross-file data flow synthesis.

    Captures N lines above/below every occurrence of 'target' across
    all files. Discards 99% of file content. LLM sees only the sparse
    interferometry array around the target phrase.
    """
    if not vgit.is_repo(path):
        return {"error": "Not a git repository."}
    path = os.path.abspath(path)
    ...
    # Build file list from git tracked files (avoid binary)
    try:
        git_files = subprocess.run(
            ["git", "ls-files"], capture_output=True, cwd=path, timeout=30, text=True, errors="replace",
        ).stdout.strip().split("\n")
    except Exception:
        git_files = []

    code_exts = {".ts", ".js", ".py", ".go", ".rs", ".c", ".cpp", ".h", ".java", ".rb", ".ex", ".exs", ".nix", ".zig", ".hs", ".jl", ".clj", ".scala"}
    target_lower = target.lower()
    arrays: list[dict] = []

    for rel_path in git_files:
        ext = rel_path.rsplit(".", 1)[-1] if "." in rel_path else ""
        if f".{ext}" not in code_exts or not ext:
            continue
        ...
        for i, line in enumerate(file_lines):
            if target_lower in line.lower():
                start = max(0, i - aperture)
                end = min(len(file_lines), i + aperture + 1)
                context = "".join(file_lines[start:end]).strip()
                arrays.append({"file": rel_path, "line": i + 1, "aperture": aperture, "context": context[:400]})
                if len(arrays) >= max_arrays:
                    break
        if len(arrays) >= max_arrays:
            break

    total_matches = len(arrays)
    if not arrays:
        return {"error": f"no occurrences of '{target}' found"}

    return {
        "target": target,
        "total_matches": total_matches,
        "arrays": arrays[:max_arrays],
        "aperture": aperture,
        "estimated_tokens": sum(len(a["context"].split()) for a in arrays[:max_arrays]),
    }


def wave_collapse_report(path: str = ".", directory: str = "",
                          out_file: str = "") -> dict:
    """Wave Function Collapse — extract convention constraints from adjacent files.

    Scans files in the same directory for state management, styling, HTTP client
    and other architectural conventions. Emits collapsed constraints.
    """
    if not vgit.is_repo(path):
        return {"error": "Not a git repository."}
    path = os.path.abspath(path)
    if not directory:
        return {"error": "provide --dir"}
    dir_full = os.path.join(path, directory)
    ...
    # Collect imports and patterns from files in this directory
    state_managers = {"zustand", "redux", "mobx", "recoil", "jotai", "valtio", "pinia", "vuex"}
    styling = {"tailwind", "css modules", "styled-components", "emotion", "sass", "less", "postcss"}
    http_clients = {"fetch", "axios", "got", "superagent", "ky", "urql", "apollo", "react-query", "swr"}
    detected = {"state": set(), "style": set(), "http": set(), "imports": set()}
        ...
        if fv.path.startswith(dir_prefix):
            ...
            for phrase in fv.vocabulary:
                ...
                for sm in state_managers:
                    if sm in pl:
                        detected["state"].add(sm)
                for st in styling:
                    if st in pl or st.replace(" ", "") in pl:
                        detected["style"].add(st)
                for hc in http_clients:
                    if hc in pl:
                        detected["http"].add(hc)
                # Collect capitalized identifiers as import hints
                import re
                for m in re.finditer(r'\b[A-Z][a-zA-Z0-9_]{3,40}\b', phrase):
                    detected["imports"].add(m.group())

    constraints: list[str] = []
    if detected["state"]:
        constraints.append(f"[State]: Must use {', '.join(sorted(detected['state'])[:2])}")
    if detected["style"]:
        constraints.append(f"[Style]: Must use {', '.join(sorted(detected['style'])[:2])}")
    if detected["http"]:
        constraints.append(f"[HTTP]: Must use {', '.join(sorted(detected['http'])[:2])}")
    imports_to_use = sorted(detected["imports"] - {"import", "export", "from", "default"})[:5]
    if imports_to_use:
        constraints.append(f"[Vocab]: Must reference {', '.join(imports_to_use)}")
    if not constraints:
        constraints.append("No dominant conventions detected in this directory.")

    return {
        "directory": directory,
        "intended_file": out_file or f"<new file in {directory}>",
        "files_scanned": dir_phrase_count,
