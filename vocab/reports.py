...
def negentropy_report(path: str = ".") -> dict:
    if not vgit.is_repo(path):
        return {"error": "Not a git repository."}
    path = os.path.abspath(path)
    from vocab.scanner import scan_codebase
    ...
    pairs = []
    for (a, b), obs in co.items():
        pa = dc.get(a, 1) / total
        pb = dc.get(b, 1) / total
        pab = obs / total
        if pab > 0 and pa * pb > 0:
            pmi = math.log2(pab / (pa * pb))
            if pmi > 1.0:
                pairs.append({"pair": [a, b], "pmi": round(pmi, 2), "co_occurrences": obs})
    pairs.sort(key=lambda x: -x["pmi"])
    return {"pairs": pairs[:8], "avg_pmi": round(sum(p["pmi"] for p in pairs) / max(len(pairs), 1), 2) if pairs else 0}


def bathymetry_report(path: str = ".", max_depth: int = 5) -> dict:
    from vocab.scanner import scan_codebase
    ...
    try:
        ...
        return {"error": f"scan failed: {e}"}
    token_re = re.compile(r'\b[A-Z][a-zA-Z0-9_]{4,40}\b')
    depth_ids: dict[str, set[int]] = {}
    for fv in analysis.file_vocabs:
        depth = min(fv.path.count("/"), max_depth)
        tokens = set()
        for phrase in fv.vocabulary:
            for m in token_re.finditer(phrase):
                tokens.add(m.group())
        for t in tokens:
            if t not in depth_ids:
                depth_ids[t] = set()
            depth_ids[t].add(depth)
    raw = subprocess.run(["git", "ls-files"], capture_output=True, cwd=path, timeout=10, text=True)
    all_files = raw.stdout.strip().split("\n") if raw.stdout else [fv.path for fv in analysis.file_vocabs]
    prof: list[float] = []
    for d in range(max_depth + 1):
        ids = sum(1 for v in depth_ids.values() if d in v)
        fc = sum(1 for f in all_files if f.count("/") >= d)
        prof.append(round(ids / max(fc, 1), 2))
    return {"profile": prof, "profile_str": " -> ".join(str(v) for v in prof)}
...
def dendro_report(path: str = ".", lookback_weeks: int = 12) -> dict:
    from vocab.scanner import scan_codebase
    import re
    if not vgit.is_repo(path):
        return {"error": "Not a git repository."}
    path = os.path.abspath(path)
    wd = vgit.weekly_commits(path, weeks=lookback_weeks)
    if len(wd) < 4:
        return {"error": "insufficient data"}
    token_re = re.compile(r'\b[A-Z][a-zA-Z0-9_]{4,40}\b')
    buckets: dict[int, set[str]] = {}
    for i, wk in enumerate(wd):
        bk = i // 4
        ...
        if not shas:
            continue
        try:
            an = scan_codebase(path, git_ref=shas[-1], quiet=True, max_files=1500, max_seconds=20)
        except Exception:
            continue
        for fv in an.file_vocabs:
            for phrase in fv.vocabulary:
                for m in token_re.finditer(phrase):
                    buckets[bk].add(m.group())
    bl = list(buckets.items())
    cohorts: list[dict] = []
    for i in range(len(bl)):
        for j in range(i + 1, len(bl)):
            ...
            surv = then & now
            if len(surv) >= 3:
                cohorts.append({"bucket": i, "survivors": sorted(surv)[:5], "count": len(surv)})
    cohorts.sort(key=lambda x: -x["count"])
    return {"cohorts": cohorts[:5]}


def vermillon_report(path: str = ".") -> dict:
    from vocab.scanner import scan_codebase
    import re, collections
    if not vgit.is_repo(path):
        return {"error": "Not a git repository."}
    ...
    total = len(analysis.file_vocabs)
    if total < 30:
        return {"error": "need >=30 files"}
    token_re = re.compile(r'\b[A-Z][a-zA-Z0-9_]{4,40}\b')
    fc: collections.Counter[str] = collections.Counter()
    for fv in analysis.file_vocabs:
        seen = set()
        for phrase in fv.vocabulary:
            for m in token_re.finditer(phrase):
        ...
        for t in seen:
            fc[t] += 1
    tiers = [(0, 0.05), (1, 0.15), (2, 0.35), (3, 0.65), (4, 1.0)]
    tc: dict[int, int] = {t: 0 for t in range(5)}
    for phrase, count in fc.items():
        pct = count / total
        for tier, boundary in tiers:
            if pct <= boundary:
                tc[tier] += 1
                break
    gaps = [t for t in zip(tc.keys(), tc.values()) if t[1] == 0]
    return {"tier_counts": tc, "missing_tiers": [t for t, c in tc.items() if c == 0]}
