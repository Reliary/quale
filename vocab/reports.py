...
def noise_cancel_report(path: str = ".", file_path: str = "",
                         focus: str = "") -> dict:
    """Active Noise Cancellation — cancel stable code to reveal only anomalies.

    Lines containing ONLY stable anchors (phrases in >80% of history) are
    'ambient noise' — cancelled from the prompt. Only unstable edges remain.
    """
    if not vgit.is_repo(path):
        return {"error": "Not a git repository."}
    path = os.path.abspath(path)
    ...
    # Build global phrase frequency to identify stable anchors
    from collections import Counter
    global_freq: Counter[str] = Counter()
    for fv in analysis.file_vocabs:
        for phrase in fv.vocabulary:
    ...
    total_files = max(len(analysis.file_vocabs), 1)
    stable_threshold = int(total_files * 0.8)

    with open(full) as f:
        lines = f.readlines()

    output_lines: list[str] = []
    noise_blocks: list[tuple[int, int]] = []
    in_noise = False
    noise_start = 0
    signal_count = 0
    noise_count = 0
    ...
    for i, line in enumerate(lines):
        ...
        if not tokens:
            continue
        # Check if ALL tokens are stable anchors
        all_stable = all(global_freq.get(t, 0) >= stable_threshold for t in tokens)
        if all_stable:
            if not in_noise:
                in_noise = True
                noise_start = i
            ...
            if in_noise:
                noise_lines = i - noise_start
                if noise_lines >= 3:
                    output_lines.append(f"// [{noise_lines} lines of STABLE NOISE CANCELLED]")
                    noise_blocks.append((noise_start + 1, i))
                in_noise = False
            output_lines.append(line.rstrip())
            signal_count += 1

    if in_noise:
        noise_lines = len(lines) - noise_start
        if noise_lines >= 3:
            output_lines.append(f"// [{noise_lines} lines of STABLE NOISE CANCELLED]")
            noise_blocks.append((noise_start + 1, len(lines)))

    return {
        "file": file_path,
        "total_lines": len(lines),
        "signal_lines": signal_count,
        "noise_lines_cancelled": noise_count,
        "noise_blocks": noise_blocks[:5],
        "compression_pct": round(noise_count / max(len(lines), 1) * 100, 1),
        "cancelled_text": "\n".join(output_lines),
        "focus": focus or None,
    }


def microdot_report(path: str = ".", class_name: str = "") -> dict:
    """Microdot Interface — compress class interface to 15-token shape.

    Extracts public method signatures from a class file. Hashes
    implementation detail away. Leaves only structural constraints.
    """
    if not vgit.is_repo(path):
        return {"error": "Not a git repository."}
    path = os.path.abspath(path)
    if not class_name:
        return {"error": "provide --class"}
    ...
    class_file = ""
    for fv in analysis.file_vocabs:
        if class_name.lower() in fv.path.lower() or class_name.lower() in str(fv.vocabulary.keys()).lower():
            if class_file and len(fv.path) < len(class_file):
                class_file = fv.path
            elif not class_file:
                class_file = fv.path

    if not class_file:
        return {"error": f"class '{class_name}' not found in any file"}

    full = os.path.join(path, class_file)
    if not os.path.exists(full):
        return {"error": f"file not found: {class_file}"}
    ...
    # Extract method-like signatures
    signatures: list[str] = []
    for line in content.split("\n"):
        stripped = line.strip()
        if any(kw in stripped for kw in ["function ", "async ", "=>", "constructor",
                                          f"class {class_name}", "interface ", "method "]):
            # Compress: keep only signature, strip body
            sig = stripped.split("{")[0].split("=>")[0].strip()[:80]
            if sig and len(sig) > 5:
                # Extract just the unique identifiers
                tokens = token_re.findall(sig)
                compact = f"({', '.join(tokens)})"
                if compact not in signatures:
                    signatures.append(compact)

    # Also extract from existing vocabulary
    for fv in analysis.file_vocabs:
        if fv.path == class_file:
            for phrase in fv.vocabulary:
                if class_name in phrase:
    ...
    microdot = f"[MICRODOT: {class_name}]: " + ", ".join(signatures[:5])
    ...
    return {
        ...
        "microdot": microdot,
        "microdot_tokens_est": len(microdot.split()),
        "original_file_tokens_est": len(content.split()),
        "compression_ratio": f"~{len(content.split()) // max(len(microdot.split()), 1)}x" if microdot else "N/A",
    }


def fractal_zoom_report(path: str = ".", focus: str = "",
                         resolution: int = 1,
                         parent_path: str = "") -> dict:
    """Mandelbrot Context — fractal sub-sampling of codebase.

    Resolution 1: module-level boundary phrases (20 tokens).
    Resolution 2: per-file phrase centroids (40 tokens).
    Resolution 3: per-function phrase centroids (80 tokens).
    LLM navigates via zoom, never calls read_file.
    """
    if not vgit.is_repo(path):
        return {"error": "Not a git repository."}
    path = os.path.abspath(path)
    ...
    if resolution <= 1:
        ...
        mod_list = mods.get("modules", []) if isinstance(mods, dict) else []
        items = []
        for m in mod_list[:10]:
            exemplars = m.get("exemplar_phrases", [])[:4]
            files = m.get("files", [])[:3]
            items.append(f"module({', '.join(exemplars)}): {', '.join(files)}")
        zoom_output = "\n".join(items) if items else "No modules found"
        return {"resolution": 1, "focus": focus or None, "zoom": zoom_output[:500],
                "navigation_hint": "Use --resolution 2 --parent-path <dir> for file-level zoom"}

    elif resolution == 2 and parent_path:
        # File level within a directory
        dir_prefix = parent_path.rstrip("/") + "/"
        items = []
        for fv in analysis.file_vocabs:
            if fv.path.startswith(dir_prefix):
                phrases = set()
                for phrase in fv.vocabulary:
                    for m in token_re.finditer(phrase):
                        phrases.add(m.group())
                if phrases:
                    items.append(f"file({fv.path}): {', '.join(sorted(phrases)[:5])}")
        zoom_output = "\n".join(items[:10]) if items else "No files found"
        return {"resolution": 2, "focus": focus or None, "parent": parent_path,
                "zoom": zoom_output[:500],
                "navigation_hint": "Use --resolution 3 --parent-path <file> for function-level zoom"}

    elif resolution == 3 and parent_path:
        # Function/block level within a single file
        full = os.path.join(path, parent_path)
        if not os.path.exists(full):
            return {"error": f"file not found: {parent_path}"}
        with open(full) as f:
            lines = f.readlines()
        blocks = _indent_blocks(lines)
        items = []
            ...
            for i in range(blk["start"], blk["end"] + 1):
                if i < len(lines):
                    for m in token_re.finditer(lines[i]):
                        phrases.add(m.group())
            if phrases:
                items.append(f"block(L{blk['start']+1}-{blk['end']+1}): {', '.join(sorted(phrases)[:4])}")
        zoom_output = "\n".join(items) if items else "No blocks found"
        return {"resolution": 3, "focus": focus or None, "parent": parent_path,
                "zoom": zoom_output[:600],
                "navigation_hint": "Use --resolution 2 --parent-path <dir> for broader context"}

    return {"error": "invalid resolution or missing --parent-path for resolution >= 2"}
